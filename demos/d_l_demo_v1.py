...
# Add tools directory to path to load Depth-Anything
...
MODEL_CHECKPOINT  = f"results/run_20260721_153135/checkpoints/best_model_20260721_153135.pt"
OUTPUT_VIDEO      = f"run{run}_comparison_multi_light.mp4"
TIME_THRESHOLD_MS = 5.0
BLACK_BACKGROUND  = False  # Set to True to draw on a clean black canvas, or False to draw over MP4 frames

# --- Granular Simulated Lighting Parameters ---
# 1. Weak Light (Severe underexposure / near pitch-blackness)
WEAK_LIGHT_ALPHA   = 0.05
WEAK_LIGHT_BETA    = 0

# 2. Small Light (Dim / very dark setting)
SMALL_LIGHT_ALPHA  = 0.18
SMALL_LIGHT_BETA   = 5

# 3. Medium Light (Moderate low-light indoor setting)
MEDIUM_LIGHT_ALPHA = 0.45
MEDIUM_LIGHT_BETA  = 15

# 4. Strong Light (High overexposure / saturation glare)
STRONG_LIGHT_ALPHA = 1.80
STRONG_LIGHT_BETA  = 80
# ==============================================================================
...
def draw_keypoints(img, landmarks, color, radius=4, show_depth=True):
    """Draws keypoints and optionally overlays depth (Z) text."""
    h, w, _ = img.shape
    for kp in landmarks:
        x = int(kp[0] * w)
        y = int(kp[1] * h)
        if 0 <= x < w and 0 <= y < h:
            ...
            if show_depth and len(kp) >= 3:
                z = kp[2]
                # Offset text slightly from the point
                cv2.putText(img, f"{z:.2f}", (x + 5, y - 5), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1, cv2.LINE_AA)


def process_and_draw(frame_to_use, radar_pred, radar_presence, radar_handedness, hands_detector, depth_estimator, title, black_bg=False, draw_mp=True):
    ...
    if draw_mp:
        ...
        if mp_results.multi_hand_landmarks:
            ...
            # Run Depth-Anything Model on the raw frame to get absolute metric depth
            from PIL import Image
            pil_img = Image.fromarray(cv2.cvtColor(frame_to_use, cv2.COLOR_BGR2RGB))
            raw_depth_map = depth_estimator.predict_depth([pil_img])[0].cpu().numpy()
            metric_depth_map = scale_depth(raw_depth_map)
            fh, fw, _ = frame_to_use.shape
            
            for lm in hand_landmarks.landmark:
                px = max(0, min(int(lm.x * fw), fw - 1))
                py = max(0, min(int(lm.y * fh), fh - 1))
                # Sample the absolute depth from DAM instead of MediaPipe's relative pseudo-depth
                z_dam = float(metric_depth_map[py, px])
                gt_landmarks.append([lm.x, lm.y, z_dam])
                
            # Draw MediaPipe predictions in Red (now with TRUE DAM depth)
            draw_keypoints(canvas, gt_landmarks, color=(0, 0, 255), radius=4)

    # Draw Radar Model Predictions in Blue
    if radar_presence > 0.5:
        draw_keypoints(canvas, radar_pred, color=(255, 0, 0), radius=4)
    ...
    if draw_mp:
        ...
        mp_color = (0, 0, 255) if mp_detected else (150, 150, 150)
        cv2.putText(canvas, mp_text, (20, 95), cv2.FONT_HERSHEY_SIMPLEX, 0.7, mp_color, 2, cv2.LINE_AA)

    cv2.putText(canvas, "MP (GT): Red", (w - 160, 35), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
    cv2.putText(canvas, "Radar Pred: Blue", (w - 160, 55), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)

    return canvas


def render_frame_grid(frame, radar_cube, model, device, hands_detector, depth_estimator, to_tensor, width, height):
    """Generates predictions and builds a 2x3 visual comparison grid."""
    
    # 1. Compute Radar Model inference (independent of lighting simulation)
    dummy_sample = {
        "radar_tensor": radar_cube,
        "keypoints_3d": np.zeros((21, 3)),
        "hand_status": np.array([0.0]),
        "handedness": np.array([-1.0])
    }
    ...
    # 2. Simulate progressive lighting conditions
    frame_weak   = cv2.convertScaleAbs(frame, alpha=WEAK_LIGHT_ALPHA, beta=WEAK_LIGHT_BETA)
    frame_small  = cv2.convertScaleAbs(frame, alpha=SMALL_LIGHT_ALPHA, beta=SMALL_LIGHT_BETA)
    frame_medium = cv2.convertScaleAbs(frame, alpha=MEDIUM_LIGHT_ALPHA, beta=MEDIUM_LIGHT_BETA)
    frame_strong = cv2.convertScaleAbs(frame, alpha=STRONG_LIGHT_ALPHA, beta=STRONG_LIGHT_BETA)

    # 3. Process and draw each grid pane
    # Panel 1: Weak Light
    panel_weak = process_and_draw(
        frame_weak, pred_landmarks, presence_score, handedness_score, 
        hands_detector, depth_estimator, "Weak Light (Low)", black_bg=BLACK_BACKGROUND, draw_mp=True
    )
    # Panel 2: Small Light
    panel_small = process_and_draw(
        frame_small, pred_landmarks, presence_score, handedness_score, 
        hands_detector, depth_estimator, "Small Light (Dim)", black_bg=BLACK_BACKGROUND, draw_mp=True
    )
    # Panel 3: Medium Light
    panel_medium = process_and_draw(
        frame_medium, pred_landmarks, presence_score, handedness_score, 
        hands_detector, depth_estimator, "Medium Light (Moderate)", black_bg=BLACK_BACKGROUND, draw_mp=True
    )
    # Panel 4: Strong Light
    panel_strong = process_and_draw(
        frame_strong, pred_landmarks, presence_score, handedness_score, 
        hands_detector, depth_estimator, "Strong Light (Overexposed)", black_bg=BLACK_BACKGROUND, draw_mp=True
    )
    # Panel 5: Normal/Original Light
    panel_normal = process_and_draw(
        frame, pred_landmarks, presence_score, handedness_score, 
        hands_detector, depth_estimator, "Normal Light", black_bg=BLACK_BACKGROUND, draw_mp=True
    )
    # Panel 6: Radar Only
    panel_radar = process_and_draw(
        frame, pred_landmarks, presence_score, handedness_score, 
        hands_detector, depth_estimator, "Dark", black_bg=True, draw_mp=False
    )

    # Combine into 2x3 grid
    top_row = np.hstack((panel_weak, panel_small, panel_medium))
    bottom_row = np.hstack((panel_strong, panel_normal, panel_radar))
...
def main():
    ...
    # Initialize Depth-Anything Model (DAM)
    dam_model_path = "/work/tuan.tt19010226/HPE-3D/tools/weights/Depth-Anything-V2-Large-hf"
    print("Loading Depth-Anything Model (DAM)...")
    depth_estimator = DepthEstimator(model_path=dam_model_path, device="cuda")
    ...
    # --- Setup Output Video Writer (2x3 Grid has 3*width and 2*height) ---
    os.makedirs(os.path.dirname(OUTPUT_VIDEO) if os.path.dirname(OUTPUT_VIDEO) else ".", exist_ok=True)
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    video_writer = cv2.VideoWriter(OUTPUT_VIDEO, fourcc, fps, (3 * width, 2 * height))
    ...
    if logs_parsed:
        ...
        for r_ts, r_idx in radar_logs:
            ...
            if best_match is not None and min_diff < threshold_seconds:
                ...
                # Render progressive grid with current frame and radar data
                grid = render_frame_grid(
                    frame, radar_cube, model, device, hands_detector, 
                    depth_estimator, to_tensor, width, height
                )
        ...
        for idx in range(max_frames):
            ...
            # Annotate current index on the grid
